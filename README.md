...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    button.reject-btn{background:transparent;border:1px solid rgba(255,100,100,0.3);padding:6px 10px;border-radius:8px;color:#ff6b6b;font-size:0.8rem;cursor:pointer}
    button.reject-btn:hover{background:rgba(255,100,100,0.1);border-color:rgba(255,100,100,0.5)}
    ...
    .modal{background:#021023;padding:16px;border-radius:12px;max-width:520px;width:100%;max-height:90vh;overflow-y:auto}
    ...
    .checkbox-item label{font-size:0.8rem;color:var(--muted);line-height:1.3}

    .resume-section{background:rgba(255,255,255,0.02);padding:12px;border-radius:8px;margin:8px 0;border:1px solid rgba(255,255,255,0.05)}
    .resume-section h5{margin:0 0 8px 0;color:#e6eef6;font-size:0.9rem}
    .resume-content{font-size:0.85rem;color:var(--muted);line-height:1.5}
    ...
    .candidate-item{background:rgba(255,255,255,0.02);padding:12px;border-radius:8px;margin-bottom:12px;border:1px solid rgba(255,255,255,0.05);transition:all 0.3s ease}
...
<body>
  <div class="wrap" role="application" aria-labelledby="title">
    <header>
      ...
      <div class="filters" aria-hidden="false">
        <button class="ghost" id="show-favs" title="Показать сохраненные вакансии">❤ Сохранённые <span id="fav-count" style="margin-left:6px;color:var(--muted)">0</span></button>
    ...
    <div class="profile-container">
      ...
      <div class="profile-dropdown" id="profileDropdown">
        <div class="dropdown-header">
          ...
          <div style="font-size:0.8rem;color:var(--muted);margin-top:4px">тинэйджерпочта@mail.ru</div>
        </div>
    ...
    <div class="controls" role="search" aria-label="Поиск работы и фильтры">
      <div class="searchbox" style="align-items:center">
        <input id="q" type="search" placeholder="Поиск работы для подростков" aria-label="Поиск вакансий" />
        ...
        <input id="location" type="search" placeholder="Локация (город/из дома)" aria-label="Местоположение" />
        <button class="btn" id="searchBtn" aria-label="Поиск">Поиск</button>
      </div>

      <div style="display:flex;flex-direction:column;gap:8px;align-items:flex-end">
        <div style="display:flex;gap:8px">
          <label style="display:flex;align-items:center;gap:6px"><small class="muted">Ваш возраст:</small>
            <input id="age" type="number" min="14" max="17" placeholder="14-17" style="width:84px" aria-label="Ваш возраст" title="Введите возраст от 14 до 17 лет" />
          </label>
    ...
    <main>
      ...
      <aside class="card" aria-labelledby="sidebar-title">
        ...
        <div>
          <h4 style="margin-bottom:6px">Советы для подростков 14-17 лет:</h4>
        ...
        <div>
          ...
          <p style="margin:0;color:var(--muted)">Мы рекомендуем указать чёткие требования к возрасту (14-17 лет) и рабочему времени!</p>
        </div>
      </aside>
    </main>
    ...
    <div class="modal" role="document">
      ...
      <div id="employerModalContent">
        <div style="text-align:center;padding:40px 20px;color:var(--muted)">
          ...
          <h3 style="color:#e6eef6;margin-bottom:12px">Отклики на вакансию</h3>
  ...
  <script>
    ...
    function updateFavsButton() {
      if (isEmployerMode) {
        $showFavs.innerHTML = '👥 Кандидаты вакансий <span id="fav-count" style="margin-left:6px;color:var(--muted)">0</span>';
        $showFavs.title = 'Просмотреть кандидатов на вакансии';
      } else {
        $showFavs.innerHTML = '❤ Сохранённые <span id="fav-count" style="margin-left:6px;color:var(--muted)">0</span>';
      ...
      card.innerHTML = `
        <div class="meta">
          ...
          <div style="display:flex;gap:8px;align-items:center;margin-top:6px">
            <small style="color:var(--muted)">${escapeHtml(job.company || job.location)}</small>
        ...
        filteredApplications.forEach((app, index) => {
          ...
          candidateElement.innerHTML = `
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
              <strong style="color:#e6eef6;font-size:1rem">${escapeHtml(app.name)}</strong>
            ...
            <div style="margin-bottom:10px">
              <div style="font-size:0.85rem;color:var(--muted);margin-bottom:4px">
                <strong style="color:#e6eef6">Email:</strong> ${escapeHtml(app.email)}
              </div>
              ${jobId ? '' : `<div style="font-size:0.85rem;color:var(--muted);margin-bottom:4px">
                <strong style="color:#e6eef6">Вакансия:</strong> ${job ? escapeHtml(job.title) : 'Неизвестная вакансия'}
              </div>`}
              <div style="font-size:0.85rem;color:var(--muted);margin-bottom:4px">
                <strong style="color:#e6eef6">Навыки:</strong> ${escapeHtml(app.skills)}
              </div>
              <div style="font-size:0.85rem;color:var(--muted);margin-bottom:4px">
                <strong style="color:#e6eef6">Отправлено:</strong> ${new Date(app.timestamp).toLocaleString('ru-RU')}
              </div>
            </div>
